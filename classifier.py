...
class Classifier(object):
    ...
    def inference(self, df, output_prefix):
        model_cls_map = joblib.load(self.cls_map_path)        
        
        for model_name in tqdm(self.ensemble_models):
            ...
            for i in range(self.tta_n):
                pred = model.predict(gen_df, verbose=1, workers=8, use_multiprocessing=True)
                pred_score = np.max(pred, axis=1)
                pred_label = np.argmax(pred, axis=1)
                pred_class = [model_cls_map[x] for x in pred_label]
                df[f'{model_name}_{i}_pred_score'] = pred_score
                df[f'{model_name}_{i}_pred_class'] = pred_class
                df[f'{model_name}_{i}_pred_class'] = df[f'{model_name}_{i}_pred_class'].astype(int)
                
        cols = df.columns
        cols = list(filter(lambda x: 'pred_class' in x, cols))
        pred_candidate = list(set([c.replace('_pred_class','') for c in cols]))    
        
        ret = df.apply(lambda r: self.get_bets_cls(r, pred_candidate), axis=1)
        df['best_cls'], df['best_score'] = zip(*ret)    
        save_dir = os.path.dirname(df.iloc[0]['filepath'])
        df[['filepath','filename','best_cls']].to_csv(os.path.join(save_dir,'pseudo_label.csv'), index=False)
        return df

    def get_bets_cls(self, r, pred_candidate):
        score_dict = {}
        cnt_dict = {}
    ...
    def get_data_gen(self, df, model_name):
        ...
        df['fake_class']='fake'
        gen_df = predGen.flow_from_dataframe(df, batch_size=10, directory=None, x_col='filepath', y_col='fake_class', shuffle=False, target_size=img_size)
        return gen_df
    ...
    def evaluate_acc(self, df):
        ...
        model_rank = self.acc_rank(df)
        print(f'Ensemble acc: {ensemble_acc:.2f}')
        print(model_rank)
